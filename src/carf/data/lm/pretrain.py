...
from src.carf.data import base
from src.carf.utils import camera 
from omegaconf import OmegaConf
from src.utils.cfg_parse import cfg_parse

class Dataset(base.Dataset):
    def __init__(self, cfg, split="train"):
        super().__init__(cfg, split)
        self.data_path = os.path.join(cfg.data.root, cfg.data.dataset, 'carf')
        
        self.split_path  =  os.path.join("splits", cfg.data.dataset, str(cfg.data.object),
                                         cfg.data.scene, f"{split}.txt")
    ...
    def initialize_meta(self, cfg):
        ...
        if self.split != 'test' and cfg.data.pose_source == 'predicted':
            if cfg.data.scene_info_source is None:
                ...
                info_names = dict(gt='scene_gt_info.json', pred='scene_pred_info.json')
    ...
    def load_camera(self, cfg, idx, obj_scene_id=0):
        ...
        if self.split == 'train' and cfg.data.pose_source == 'predicted':
            ...
            pose_pred_raw[:3, 3] = torch.from_numpy(np.array(tra).astype(np.float32)) / 1000  # scale in meter

            pose_pred = self.parse_raw_camera(cfg, pose_pred_raw)
        else:
            pose_pred = pose_gt
    ...
    def __getitem__(self, idx):
        ...
        _, intr, pose_gt, pose_pred = self.cameras[idx] if cfg.data.preload else self.load_camera(cfg, idx, obj_scene_id=obj_scene_id)
        ...
        if cfg.data.erode_mask_loss is not None:
            erode_mask = self.load_obj_mask(cfg, idx, return_erode=True)
...
if __name__ == "__main__":
    config = OmegaConf.load("configs/lm/nerf_pretrain.yaml")
    config = cfg_parse(config)
    dataset = Dataset(config, split="train")
    data_loader = dataset.setup_loader(config, shuffle=False)
