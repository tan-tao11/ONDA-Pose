...
from ..utils.mask_utils import binary_mask_to_rle, cocosegm2mask
from ..utils.utils import dprint, iprint, lazy_property
...
class LM_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        if data_cfg.get('rgb_type') is not None:
            self.rgb_type = data_cfg['rgb_type']
        else:
            self.rgb_type = 'rgb'
        ##################################################
    ...
    def __call__(self):  # LM_Dataset
        ...
        for ann_file, scene_root, xyz_root in zip(tqdm(self.ann_files), self.image_prefixes, self.xyz_prefixes):
            ...
            for im_id in tqdm(indices):
                ...
                rgb_path = osp.join(scene_root, "{}/{:06d}.png").format(self.rgb_type, int_im_id)
                assert osp.exists(rgb_path), rgb_path

                depth_path = osp.join(scene_root, "depth/{:06d}.png".format(int_im_id))
                rgb_syn_path = osp.join(scene_root, "rgb_syn/{:06d}.png".format(int_im_id))
                mask_syn_path = osp.join(scene_root, "rgb_syn/{:06d}_mask.png".format(int_im_id))
...
for obj in ref.lm_full.objects:
    for split in ["train", "test", "pred_init"]:
        name = "lm_real_{}_{}".format(obj, split)
        ann_files = [
            osp.join(
                DATASETS_ROOT,
                "lm/estimator/image_set/{}_{}.txt".format(obj, split),
            )
        ]
        if split in ["train"]:  # all is used to train lmo
            filter_invalid = True
        elif split in ["test", "pred_init"]:
        ...
        if name not in SPLITS_LM:
            SPLITS_LM[name] = dict(
                name=name,
                dataset_root=osp.join(DATASETS_ROOT, "lm/estimator/"),
                models_root=osp.join(DATASETS_ROOT, "lm/estimator/models"),
                objs=[obj],  # only this obj
                ann_files=ann_files,
                image_prefixes=[osp.join(DATASETS_ROOT, "lm/estimator/test/{:06d}").format(ref.lm_full.obj2id[obj])],
                xyz_prefixes=[
                    osp.join(
                        DATASETS_ROOT,
                        "lm/estimator/test/xyz_crop/{:06d}".format(ref.lm_full.obj2id[obj]),
                    )
                ],
...
for obj in ref.lmo_full.objects:
    for split in ["train"]:
        ...
        if name not in SPLITS_LM:
            SPLITS_LM[name] = dict(
                name=name,
                dataset_root=osp.join(DATASETS_ROOT, "lmo/estimator/"),
                models_root=osp.join(DATASETS_ROOT, "lmo/estimator/models"),
                objs=[obj],
                ann_files=[
                    osp.join(
                        DATASETS_ROOT,
                        "lmo/estimator/image_set/lmo_no_bop_test_{}.txt".format(obj),
                    )
                ],
                # NOTE: scene root
                image_prefixes=[osp.join(DATASETS_ROOT, "lmo/estimator/test/{:06d}").format(2)],
                xyz_prefixes=[
                    osp.join(
                        DATASETS_ROOT,
                        "lmo/estimator/test/xyz_crop/{:06d}".format(2),
                    )
                ],
                scale_to_meter=0.001,
                with_masks=True,  # (load masks but may not use it)
        ...
        if name not in SPLITS_LM:
            SPLITS_LM[name] = dict(
                ...
                ann_files=[
                    osp.join(
                        DATASETS_ROOT,
                        "lmo/estimator/image_set/lmo_test.txt",
                    )
                ],
                # NOTE: scene root
                image_prefixes=[osp.join(DATASETS_ROOT, "lmo/estimator/test/{:06d}").format(2)],
        ...
        if name not in SPLITS_LM:
            SPLITS_LM[name] = dict(
                ...
                ann_files=[
                    osp.join(
                        DATASETS_ROOT,
                        "lmo/estimator/image_set/lmo_bop_test.txt",
                    )
                ],
                # NOTE: scene root
                image_prefixes=[osp.join(DATASETS_ROOT, "lmo/estimator/test/{:06d}").format(2)],
